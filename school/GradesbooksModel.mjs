...
class GradesbooksModel {
  ...
  async read(id, pupil) {
    const records = this.gradeBook
      .get(id)
      .records.filter(record => record.pupilId === pupil);
    const {
      name: { first, last }
    } = await this.groups.pupil.read(records[0].pupilId);
    const result = { name: `${first} ${last}`, records: [] };

    for (const { teacherId, subjectId, lesson, mark } of records) {
      const {
        name: { first, last }
      } = await this.teachers.read(teacherId);
  ...
  async readAll(id) {
    ...
    for (const { pupilId, teacherId, subjectId, lesson, mark } of records) {
      ...
      result
        .get(pupilId)
        .records.push({ teacher: `${first} ${last}`, subject, lesson, mark });
    }
